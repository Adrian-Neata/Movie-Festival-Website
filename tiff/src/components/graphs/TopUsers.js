...
export default function TopMovies(props) {
    ...
    const [yKey, setYKey] = useState('posts')
    ...
    const getDataFromServer = async (url) => {
        ...
        axios({
            method: 'get',
            url: 'http://localhost:5000/data/top_users/' + url,
            headers: {
    ...
    if (props.category === 'posts' && yKey !== 'posts') {
        setYKey('posts')
  ...
  return (
      <div>
      <Typography align='center' style={{fontWeight: 'bold'}}>Top Users by {yKey}</Typography>
